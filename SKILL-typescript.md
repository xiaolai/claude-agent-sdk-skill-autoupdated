# Claude Agent SDK — TypeScript Reference (v0.2.132)


**Package**: `@anthropic-ai/claude-agent-sdk@0.2.132`
...
await q.getContextUsage();                  // Get context window usage breakdown by category — returns SDKControlGetContextUsageResponse (v0.2.132)
...
type SDKMessage =
  ...
  | SDKAPIRetryMessage            // type: 'system', subtype: 'api_retry' — transient API error being retried (v0.2.132)
...
| v0.2.132 | `planModeInstructions` added to `SDKSessionOptions` — now supported in V2 session API (`unstable_v2_createSession`, `unstable_v2_resumeSession`) |
| v0.2.132 | Added `network.allowMachLookup` sandbox option (macOS only — allows XPC/Mach service lookups needed for Playwright, iOS Simulator, Go-based tools with MITM proxy) |
| v0.2.132 | Added `PermissionDenied` hook event (29 total as of v0.2.132) |
| v0.2.132 | Added `Query.getContextUsage()` method (context window breakdown by category); made `SDKUserMessage.session_id` optional; added `@anthropic-ai/sdk` and `@modelcontextprotocol/sdk` as explicit dependencies (fixes type-any regression) |
...
**Last verified**: 2026-05-07 | **SDK version**: 0.2.132
