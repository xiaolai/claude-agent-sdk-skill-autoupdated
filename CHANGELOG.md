...
- No version bump (TS v0.2.126 and PY v0.1.72 unchanged); monitor detected registry dedup artifacts only
- TS: Known Issue #10 expanded — added separate 60s hardcoded timeout for HTTP MCP POST requests (issue #297); PY: date refresh only
- Typecheck false-positive (recurring stale-comma in import check); verify passed 2/2 on attempt 1 with 0 mend runs
- [Full report](reports/2026-05-03.md)
...
- PY: added KI #42 (`TaskNotificationMessage` leaks into next `receive_response()` turn from background tasks); added KI #43 (`context-management` beta header breaks OpenRouter/third-party providers in SDK ≥ v0.1.46)
- Typecheck false-positive (recurring stale-comma import artifact); mended in 1 pass; verify passed 18/18 on attempt 2
- [Full report](reports/2026-04-12.md)

## 2026-04-11

- SDK TS v0.2.98 → v0.2.101; PY v0.1.58 unchanged
- TS: added `SDKTaskUpdatedMessage` (task_updated subtype), `unattributedTokens` in context usage; SDK now emits 25 message types
- PY: corrected KI #23 version wording; added missing v0.1.57 changelog entry (thinking-flag fix, `auto` PermissionMode)
- Typecheck false-positive (recurring stale-comma import artifact); mended in 1 pass; verify passed 18/18 on attempt 2
- [Full report](reports/2026-04-11.md)
