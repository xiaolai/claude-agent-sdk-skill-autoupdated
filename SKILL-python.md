# Claude Agent SDK — Python Reference (v0.1.58)

**Package**: `claude-agent-sdk==0.1.58` (PyPI)
...
| **Custom Tools**    | Yes (v0.1.58+: string prompts fully supported; older versions require AsyncIterable — see [#14](#14-sdk-mcp-servers-completely-non-functional-with-string-prompts)) | Yes |
...
2. **With per-parameter descriptions** using `typing.Annotated` (added v0.1.58):
   ```python
...
**`annotations` parameter** — pass `ToolAnnotations` (re-exported from `mcp.types`) to set MCP tool metadata. The `maxResultSizeChars` attribute controls the CLI's spill threshold for large tool results (fixed in v0.1.58, [#756](https://github.com/anthropics/claude-agent-sdk-python/issues/756)):
...
class AssistantMessage:
    ...
    message_id: str | None = None          # Anthropic API message ID (added v0.1.58)
    stop_reason: str | None = None         # Raw stop reason from Anthropic API (added v0.1.58)
    session_id: str | None = None          # Session this message belongs to (added v0.1.58)
    uuid: str | None = None                # Unique message identifier (added v0.1.58)
...
class ResultMessage:
    ...
    model_usage: dict[str, Any] | None = None       # Per-model token breakdown (added v0.1.58)
    permission_denials: list[Any] | None = None     # Permission denial records (added v0.1.58)
    errors: list[str] | None = None                 # Error messages from the session (added v0.1.58)
    uuid: str | None = None                         # Unique message identifier (added v0.1.58)
```
...
class AgentDefinition:
    ...
    background: bool | None = None                           # Run agent in the background (added v0.1.58)
    effort: Literal["low", "medium", "high", "max"] | int | None = None  # Effort level for thinking depth (added v0.1.58)
    permissionMode: PermissionMode | None = None             # Permission mode for this agent (added v0.1.58)
```
...
**Fix** (v0.1.58): `CLAUDECODE` is now automatically filtered from the subprocess environment (PR [#732](https://github.com/anthropics/claude-agent-sdk-python/issues/732)). Upgrade to v0.1.58+ to resolve. On v0.1.50, override manually:
```python
options = ClaudeAgentOptions(
    env={"CLAUDECODE": ""},
    # ... other options
...
**Fix** (v0.1.58): `resource_link`, `embedded_resource`, and `audio` content types are now handled (PR [#725](https://github.com/anthropics/claude-agent-sdk-python/issues/725)). The `search_result` type remains unsupported.
...
### #14: SDK MCP Servers Completely Non-functional with String Prompts (Fixed in v0.1.58)
...
**Fix** (v0.1.58): Root causes (1) and (2) are fully resolved — `query()` now always calls `initialize()` (sending `sdkMcpServers` to the CLI) and spawns `wait_for_result_and_end_input()` as a background task instead of awaiting it inline (PR [#780](https://github.com/anthropics/claude-agent-sdk-python/pull/780)). Root cause (3) was fixed in v0.1.58 ([#731](https://github.com/anthropics/claude-agent-sdk-python/issues/731)). Upgrade to v0.1.58+ — string prompts fully work with SDK MCP servers.
**Workaround (pre-v0.1.58 only)**: Use `AsyncIterable` prompt instead of string:
```python
...
### #29: SDK MCP Server Tool Errors Not Propagated (`isError` Field Mismatch) (Fixed in v0.1.58)
...
**Fix** (v0.1.58): `is_error` is now correctly propagated to the CLI (PR [#717](https://github.com/anthropics/claude-agent-sdk-python/issues/717)). Upgrade to v0.1.58+.
...
### #30: `query()` with Hooks Closes stdin After 60s, Killing Hook Callbacks (Fixed in v0.1.58)
...
**Fix** (v0.1.58): stdin timeout is removed when hooks or SDK MCP servers are present (PR [#731](https://github.com/anthropics/claude-agent-sdk-python/issues/731)). Upgrade to v0.1.58+.
...
### #31: `CLAUDE_CODE_STREAM_CLOSE_TIMEOUT` Ignored in `query()` Initialize Timeout (Fixed in v0.1.58)
...
**Fix** (v0.1.58): `CLAUDE_CODE_STREAM_CLOSE_TIMEOUT` is now propagated to `query()` as well (PR [#743](https://github.com/anthropics/claude-agent-sdk-python/issues/743)). Upgrade to v0.1.58+.
...
### #33: `AssistantMessage` and `ResultMessage` Drop Significant Fields (Fixed in v0.1.58)
...
**Fix** (v0.1.58): `AssistantMessage` now exposes `message_id`, `stop_reason`, `session_id`, and `uuid`. `ResultMessage` now exposes `model_usage`, `permission_denials`, `errors`, and `uuid` (PR [#718](https://github.com/anthropics/claude-agent-sdk-python/issues/718)). Upgrade to v0.1.58+.
...
### #34: `shutil.which()` Blocking Call Raises Error in Strict Async Environments (Fixed in v0.1.58)
...
**Fix** (v0.1.58): CLI discovery is now deferred to `connect()` time and is non-blocking (PR [#722](https://github.com/anthropics/claude-agent-sdk-python/issues/722)). Upgrade to v0.1.58+.
...
### #35: `connect(prompt=str)` Silently Drops Prompt — `receive_messages()` Hangs (Fixed in v0.1.58)
...
**Fix** (v0.1.58): String prompts are now wrapped in a user message and sent to stdin in `connect()` (PR [#769](https://github.com/anthropics/claude-agent-sdk-python/pull/769)). Upgrade to v0.1.58+.
**Workaround** (pre-v0.1.58): Use `connect()` without a prompt, then call `query()` separately:
```python
# WRONG on pre-v0.1.58 — prompt silently dropped
...
| Version | Change |
|---------|--------|
| v0.1.58 | Bundled CLI updated to v2.1.92 (no API changes) |
| v0.1.58 | MCP large tool results: `ToolAnnotations.maxResultSizeChars` now correctly forwarded to CLI via `_meta`, fixing silent truncation of large tool results ([#756](https://github.com/anthropics/claude-agent-sdk-python/issues/756)); bundled CLI updated to v2.1.91 |
| v0.1.58 | `SessionStartHookSpecificOutput` type added to `HookSpecificOutput` union in preparation for future `SessionStart` hook event (not yet in `HookEvent` union) |
| v0.1.58 | `AgentDefinition` gains `background`, `effort`, and `permissionMode` fields; `list_sessions()` gains `offset` parameter |
