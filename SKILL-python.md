...
| **Custom Tools**    | Yes (v0.2.82+: string prompts fully supported; older versions require AsyncIterable — see [#14](#14-sdk-mcp-servers-completely-non-functional-with-string-prompts)) | Yes |
...
2. **With per-parameter descriptions** using `typing.Annotated` (added v0.2.82):
   ```python
...
**`annotations` parameter** — pass `ToolAnnotations` (re-exported from `mcp.types`) to set MCP tool metadata. The `maxResultSizeChars` attribute controls the CLI's spill threshold for large tool results (fixed in v0.2.82, [#756](https://github.com/anthropics/claude-agent-sdk-python/issues/756)):
...
class AssistantMessage:
    ...
    message_id: str | None = None          # Anthropic API message ID (added v0.2.82)
    stop_reason: str | None = None         # Raw stop reason from Anthropic API (added v0.2.82)
    session_id: str | None = None          # Session this message belongs to (added v0.2.82)
    uuid: str | None = None                # Unique message identifier (added v0.2.82)
...
class ResultMessage:
    ...
    model_usage: dict[str, Any] | None = None       # Per-model token breakdown (added v0.2.82)
    permission_denials: list[Any] | None = None     # Permission denial records (added v0.2.82)
    ...
    errors: list[str] | None = None                 # Error messages from the session (added v0.2.82)
...
class AgentDefinition:
    ...
    background: bool | None = None                           # Run agent in the background (added v0.2.82)
    effort: Literal["low", "medium", "high", "xhigh", "max"] | int | None = None  # Effort level for thinking depth (added v0.2.82; "xhigh" = extended depth on Opus 4.7+)
    permissionMode: PermissionMode | None = None             # Permission mode for this agent (added v0.2.82)
```
...
# Controls whether thinking text is returned summarized or omitted (added v0.2.82).
...
The `claude_agent_sdk.testing` subpackage provides a shared conformance suite for `SessionStore` adapters. Use it to verify your implementation satisfies all 14 behavioral contracts required by the SDK (added v0.2.82).
...
**Fix** (v0.2.82): `CLAUDECODE` is now automatically filtered from the subprocess environment (PR [#732](https://github.com/anthropics/claude-agent-sdk-python/issues/732)). Upgrade to v0.2.82+ to resolve. On v0.1.50, override manually:
```python
options = ClaudeAgentOptions(
    env={"CLAUDECODE": ""},
    # ... other options
...
**Fix** (v0.2.82): `resource_link`, `embedded_resource`, and `audio` content types are now handled (PR [#725](https://github.com/anthropics/claude-agent-sdk-python/issues/725)). The `search_result` type remains unsupported.
...
### #14: SDK MCP Servers Completely Non-functional with String Prompts (Fixed in v0.2.82)
...
**Fix** (v0.2.82): Root causes (1) and (2) are fully resolved — `query()` now always calls `initialize()` (sending `sdkMcpServers` to the CLI) and spawns `wait_for_result_and_end_input()` as a background task instead of awaiting it inline (PR [#780](https://github.com/anthropics/claude-agent-sdk-python/pull/780)). Root cause (3) was fixed in v0.2.82 ([#731](https://github.com/anthropics/claude-agent-sdk-python/issues/731)). Upgrade to v0.2.82+ — string prompts fully work with SDK MCP servers.
**Workaround (pre-v0.2.82 only)**: Use `AsyncIterable` prompt instead of string:
```python
...
### #29: SDK MCP Server Tool Errors Not Propagated (`isError` Field Mismatch) (Fixed in v0.2.82)
...
**Fix** (v0.2.82): `is_error` is now correctly propagated to the CLI (PR [#717](https://github.com/anthropics/claude-agent-sdk-python/issues/717)). Upgrade to v0.2.82+.
...
### #30: `query()` with Hooks Closes stdin After 60s, Killing Hook Callbacks (Fixed in v0.2.82)
...
**Fix** (v0.2.82): stdin timeout is removed when hooks or SDK MCP servers are present (PR [#731](https://github.com/anthropics/claude-agent-sdk-python/issues/731)). Upgrade to v0.2.82+.
...
### #31: `CLAUDE_CODE_STREAM_CLOSE_TIMEOUT` Ignored in `query()` Initialize Timeout (Fixed in v0.2.82)
...
**Fix** (v0.2.82): `CLAUDE_CODE_STREAM_CLOSE_TIMEOUT` is now propagated to `query()` as well (PR [#743](https://github.com/anthropics/claude-agent-sdk-python/issues/743)). Upgrade to v0.2.82+.
...
### #33: `AssistantMessage` and `ResultMessage` Drop Significant Fields (Fixed in v0.2.82)
...
**Fix** (v0.2.82): `AssistantMessage` now exposes `message_id`, `stop_reason`, `session_id`, and `uuid`. `ResultMessage` now exposes `model_usage`, `permission_denials`, `errors`, and `uuid` (PR [#718](https://github.com/anthropics/claude-agent-sdk-python/issues/718)). Upgrade to v0.2.82+.
...
### #34: `shutil.which()` Blocking Call Raises Error in Strict Async Environments (Fixed in v0.2.82)
...
**Fix** (v0.2.82): CLI discovery is now deferred to `connect()` time and is non-blocking (PR [#722](https://github.com/anthropics/claude-agent-sdk-python/issues/722)). Upgrade to v0.2.82+.
...
### #35: `connect(prompt=str)` Silently Drops Prompt — `receive_messages()` Hangs (Fixed in v0.2.82)
...
**Fix** (v0.2.82): String prompts are now wrapped in a user message and sent to stdin in `connect()` (PR [#769](https://github.com/anthropics/claude-agent-sdk-python/pull/769)). Upgrade to v0.2.82+.
**Workaround** (pre-v0.2.82): Use `connect()` without a prompt, then call `query()` separately:
```python
# WRONG on pre-v0.2.82 — prompt silently dropped
...
| Version | Change |
|---------|--------|
| v0.2.82 | Bundled CLI updated to v2.1.142 (no Python API changes). `mcp` dependency floor raised to `>=1.23.0`. Python 3.13 now officially supported. |
| v0.2.82 | Patch release; no Python API changes. `permissionDecision: "defer"` added to `PreToolUseHookSpecificOutput` — stops the run and exposes the deferred call in `ResultMessage.deferred_tool_use` for human-in-the-loop approval workflows. |
| v0.2.82 | Bundled CLI updated to v2.1.138 (no Python API changes) |
...
| v0.2.82 | Bundled CLI updated to v2.1.133 (no Python API changes) |
| v0.2.82 | `strict_mcp_config` option added to `ClaudeAgentOptions` (bool, default `False`) — ignore all other MCP sources and use only `mcp_servers`; `include_hook_events` option added (bool, default `False`) — emit `HookEventMessage` objects into the message stream; `effort` extended with `"xhigh"` level for Opus 4.7+; `ToolPermissionContext` gains `blocked_path`, `decision_reason`, `title`, `display_name`, `description` fields; `ResultMessage` gains `deferred_tool_use` (for `permissionDecision: "defer"` hooks) and `api_error_status` (HTTP status of failing API call); `PostToolUseHookSpecificOutput` gains `updatedToolOutput` (works for all tools, not just MCP) |
| v0.2.82 | `session_store_flush` option added to `ClaudeAgentOptions` (`SessionStoreFlushMode`: `"batched"` \| `"eager"`, default `"batched"`) — controls when mirrored entries are flushed to `session_store`; bundled CLI updated to v2.1.128 |
| v0.2.82 | Bundled CLI updated to v2.1.126 (no Python API changes); `mcp` dependency floor raised to `>=1.19.0` |
| v0.2.82 | `SandboxNetworkConfig` extended with `allowedDomains`, `deniedDomains`, `allowManagedDomainsOnly`, and `allowMachLookup` fields for finer-grained network sandbox control |
| v0.1.70 | Minor patch release (no Python API changes) |
| v0.2.82 | Bundled CLI updated to v2.1.121 (no Python API changes) |
| v0.2.82 | `claude_agent_sdk.testing` subpackage added — exports `run_session_store_conformance()`, a shared conformance suite that validates all 14 `SessionStore` behavioral contracts; works with any async test runner (no pytest required); `fold_session_summary()` signature updated to `(prev, key, entries)` — `key: SessionKey` is now a required second argument; bundled CLI updated to v2.1.119 |
| v0.2.82 | `ThinkingDisplay = Literal["summarized", "omitted"]` type added (Opus 4.7+ defaults to `"omitted"`); optional `display` field added to `ThinkingConfigAdaptive` and `ThinkingConfigEnabled`; `import_session_to_store()` async function exported — replays a local on-disk session into a `SessionStore` for migration or gap-recovery after `MirrorErrorMessage` |
| v0.2.82 | `SessionSummaryEntry` type and `fold_session_summary()` helper added — enables efficient incremental summary maintenance in `SessionStore.list_session_summaries()` without full JSONL re-reads; `SessionStore` Protocol gains optional `list_session_summaries()` method; `ServerToolUseBlock` and `ServerToolResultBlock` content block types exposed (server-side tool events the API executes on the model's behalf); `ServerToolName` literal type added |
| v0.2.82 | Session Store API added: `SessionStore` Protocol, `InMemorySessionStore`, `project_key_for_directory`; store-backed session read/mutation functions (`list_sessions_from_store`, `get_session_info_from_store`, `get_session_messages_from_store`, `list_subagents_from_store`, `get_subagent_messages_from_store`, `rename_session_via_store`, `tag_session_via_store`, `delete_session_via_store`, `fork_session_via_store`); `session_store` and `load_timeout_ms` options added to `ClaudeAgentOptions`; `MirrorErrorMessage` emitted when store `append()` fails |
| v0.2.82 | Bundled CLI updated to v2.1.114 (no Python API changes) |
...
**Last verified**: 2026-05-15 | **SDK version**: 0.2.82